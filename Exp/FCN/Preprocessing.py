...
def generate_train_image(img_folder_path, file_size):
    ...
    #             # print(j)
    #             if j == False:
    #                 real_Y_train[count][x_temp][y_temp][:] = [255, 255, 255]
    #     count += 1
...
def map_y_train(Y_train, file_size, Y_ids):
    real_Y_train = np.zeros((len(Y_ids) * 7, file_size, file_size, 3), dtype=np.float32)
    count = 0;
    for item in Y_train:
        mask = item[:, :, 0]
        for length in range(0, file_size):
            for height in range(0, file_size):
                if mask[length][height]:
                    real_Y_train[count][length][height] = [0, 0, 0]
                else:
                    real_Y_train[count][length][height] = [255, 99, 59]

    return real_Y_train


if __name__ == '__main__':
    X_ids,Y_ids=load_dataset("C:\\Users\\Tom-G\\OneDrive\\3DOWN\\Bioinformatics\\Exp\FCN\\ISBI_Dataset")
    X_train,Y_train=generate_train_image("C:\\Users\\Tom-G\\OneDrive\\3DOWN\\Bioinformatics\\Exp\FCN\\ISBI_Dataset", 224)
